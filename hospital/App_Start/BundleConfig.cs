using System.Web;
using System.Web.Optimization;

namespace hospital
{
    public class BundleConfig
    {
        // 如需統合的詳細資訊，請瀏覽 https://go.microsoft.com/fwlink/?LinkId=301862
        public static void RegisterBundles(BundleCollection bundles)
        {
            bundles.Add(new ScriptBundle("~/bundles/jquery").Include(
                        "~/Scripts/jquery-{version}.js"));

            bundles.Add(new ScriptBundle("~/bundles/jqueryval").Include(
                        "~/Scripts/jquery.validate*"));

            //自行加入的template 需要的js
            //Required Js
            bundles.Add(new ScriptBundle("~/bundles/Required").Include(
                        "~/Content/assets/js/vendor-all.min.js",
                        "~/Content/assets/plugins/bootstrap/js/bootstrap.min.js"));
            bundles.Add(new ScriptBundle("~/bundles/pcoded").Include(
                        "~/Content/assets/js/pcoded.min.js"));

            // 使用開發版本的 Modernizr 進行開發並學習。然後，當您
            // 準備好可進行生產時，請使用 https://modernizr.com 的建置工具，只挑選您需要的測試。
            bundles.Add(new ScriptBundle("~/bundles/modernizr").Include(
                        "~/Scripts/modernizr-*"));

            bundles.Add(new ScriptBundle("~/bundles/bootstrap").Include(
                      "~/Scripts/bootstrap.js"));

            bundles.Add(new StyleBundle("~/Content/css").Include(
                      "~/Content/bootstrap.css",
                      "~/Content/site.css"));

            //自行加入的template 需要的css
            //Favicon icon
            bundles.Add(new StyleBundle("~/Content/assets/icon").Include(
                       "~/Content/assets/images/favicon.ico"));
            //fontawesome icon
            bundles.Add(new StyleBundle("~/Content/assets/fontawesome").Include(
                       "~/Content/assets/fonts/fontawesome/css/fontawesome-all.min.css"));
            //animation css
            bundles.Add(new StyleBundle("~/Content/assets/animation").Include(
                       "~/Content/assets/plugins/animation/css/animate.min.css"));
            //vendor css
            bundles.Add(new StyleBundle("~/Content/assets/css").Include(
                       "~/Content/assets/css/style.css"));


            bundles.Add(new ScriptBundle("~/Content/bootstrap-tagsinput").Include(
                      "~/Content/bootstrap-tagsinput/bootstrap-tagsinput.js"));
        }
    }
}
